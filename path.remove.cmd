...
:: 4. 将其中的 `;%ipzu_handy_tool_win%;` 替换为空的分号.
::
::   - 注意大小写不敏感的话可以改成:
::     set "temp=!temp:;%IPZU_HANDY_TOOL_WIN%;=;!"
::     但理论上 PATH 内部会存成 `%ipzu_handy_tool_win%`（小写）,
::   - 如果你不确定大小写，就可以 `findstr /i` 判断,
::     这里先按上面添加时的写法，使用小写替换.
::
set "temp=!temp:;%%ipzu_handy_tool_win%%;=;!"

:: 如果不确定还可能出现 `%ipzu_handy_tool_win%` 在 PATH 的开头/结尾等情况.
:: 也可以多做几次替换:
::   set "temp=!temp:;%ipzu_handy_tool_win% =;!"
::   set "temp=!temp: %ipzu_handy_tool_win%;=;!"
:: （具体情况看你添加时的逻辑而定）.
...
:: 7. 更新 PATH
setx PATH "!temp!" >nul
echo [完成] 已从 PATH 中移除 "%%ipzu_handy_tool_win%%" 引用.
echo [INFO] 新的 PATH:
echo !temp!
echo.
...
echo.
echo 2.
:: 8. 删除 ipzu_handy_tool_win 这个变量本身.
reg delete "HKCU\Environment" /v "ipzu_handy_tool_win" /f >nul 2>nul
echo [完成] 已删除用户环境变量 %%ipzu_handy_tool_win%%
